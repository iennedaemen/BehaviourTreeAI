...
void Plugin::DllInit()
{
	...
	m_pBlackboard->AddData("IgnoreItemsPos", std::vector<Elite::Vector2>());

	// agent
	m_pBlackboard->AddData("Agent", AgentInfo());
	m_pBlackboard->AddData("Steering", SteeringPlugin_Output());
	m_pBlackboard->AddData("FleeTime", 0.0f);
	m_pBlackboard->AddData("FleeingWithGun", false);
	m_pBlackboard->AddData("FleeingWithoutGun", false);
	...
	std::vector<std::shared_ptr<Behavior>> pBehaviorIgnoreItem{
	std::make_shared<BehaviorConditional>(HasItem),
	std::make_shared<BehaviorAction>(IgnoreItem)
	};
	...
	// flee
	std::vector<std::shared_ptr<Behavior>> pBehaviorFleeWithGun{
	std::make_shared<BehaviorConditional>(ShouldFleeWithGun),
	std::make_shared<BehaviorAction>(Flee)
	};

	std::vector<std::shared_ptr<Behavior>> pBehaviorFleeWithoutGun{
	std::make_shared<BehaviorConditional>(HasNoPistol),
	std::make_shared<BehaviorConditional>(ShouldFleeWithoutGun),
	std::make_shared<BehaviorAction>(Flee)
	};
	...
	// reset
	std::vector<std::shared_ptr<Behavior>> pBehaviorResetHouses{
	std::make_shared<BehaviorConditional>(ReachedCheckpoint),
	std::make_shared<BehaviorAction>(ResetHouses),
	std::make_shared<BehaviorAction>(ResetIgnoreItems)
	};
	...
	m_pBehaviorTree = std::make_shared<BehaviorTree>(m_pBlackboard, std::make_shared<BehaviorSelector>(std::vector<std::shared_ptr<Behavior>>{
		std::make_shared<BehaviorSequence>(pBehaviorResetHouses),
		...
		std::make_shared<BehaviorSelector>(std::vector<std::shared_ptr<Behavior>>{
			...
			std::make_shared<BehaviorSequence>(pBehaviorIgnoreItem)
		}),

		std::make_shared<BehaviorSelector>(std::vector<std::shared_ptr<Behavior>>{
			std::make_shared<BehaviorSequence>(pBehaviorFleeWithGun),
			std::make_shared<BehaviorSequence>(pBehaviorFleeWithoutGun),
...
SteeringPlugin_Output Plugin::UpdateSteering(float dt)
{
	...
	// FLEE
	///////////////////////////////////////
	bool fleeingWithGun = false;
	m_pBlackboard->GetData("FleeingWithGun", fleeingWithGun);
	if (amountEnemiesFOV > 2 ) fleeingWithGun = true;
	else fleeingWithGun = false;

	bool fleeingWithoutGun = false;
	m_pBlackboard->GetData("FleeingWithoutGun", fleeingWithoutGun);
	if (amountEnemiesFOV > 0) fleeingWithoutGun = true;
	else fleeingWithoutGun = false;

	float fleeTime;
	m_pBlackboard->GetData("FleeTime", fleeTime);
	fleeTime -= dt;
	if(fleeingWithGun || fleeingWithoutGun)
	{
		fleeTime = 3.0f;
	}

	m_pBlackboard->ChangeData("FleeTime", fleeTime);
	m_pBlackboard->ChangeData("FleeingWithGun", fleeingWithGun);
	m_pBlackboard->ChangeData("FleeingWithoutGun", fleeingWithoutGun);
